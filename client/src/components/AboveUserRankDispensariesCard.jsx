...
const AboveUserRankDispensariesCard = () => {
  ...
  const aboveUserRank = competitors.slice(0, 10);
  ...
  return (
    ...
    >
      <CardContent>
        ...
        >
          Top 10 Competitors Above User's Rank
          <IconButton color='inherit'>
            <SettingsIcon />
          </IconButton>
        </Typography>
        <List>
          {aboveUserRank.length > 0 ? (
            aboveUserRank.map((competitor, index) => (
              <ListItem key={index}>
                <ListItemIcon>
...
export default AboveUserRankDispensariesCard;
