...
import { createPlaceholderFiles, createTimestampedDir, initSetup, verifySetup } from "./helperUtility.js";
...
const runTools = async () => {
  ...
  if (verifySetup()) {
    try {
      ...
      const archiveOutput = await executeScript("H:\\DigitalHerencia\\.environment\\DigitalHerencia_CompetitiveAdvantage\\server\\utils\\archiveUtility.js", archDir);
      const evalOutput = await executeScript("H:\\DigitalHerencia\\.environment\\DigitalHerencia_CompetitiveAdvantage\\server\\utils\\evalUtility.js", evalDir);
    ...
    logError("Helper function failed to set up the output directory and files after 5 attempts.");
    throw new Error("Helper function failed to set up the output directory and files after 5 attempts.");
  }
};
