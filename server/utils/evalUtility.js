...
const countFunctions = (content) => (content.match(/function\s+\w+/g) || []).length;
const countClasses = (content) => (content.match(/class\s+\w+/g) || []).length;
const countImports = (content) => (content.match(/import\s+/g) || []).length;
const countSingleLineComments = (content) => (content.match(/\/\/.*/g) || []).length;
const countMultiLineComments = (content) => (content.match(/\/\*[\s\S]*?\*\//g) || []).length;
...
const meetsCleanCodeCriteria = (stats) => {
  return {
    maxFunctionLength: stats.totalLines <= cleanCodeCriteria.maxFunctionLength,
    maxClassesPerFile: stats.numberOfClasses <= cleanCodeCriteria.maxClassesPerFile,
    maxImportsPerFile: stats.numberOfImports <= cleanCodeCriteria.maxImportsPerFile,
    maxSingleLineComments: stats.numberOfSingleLineComments <= cleanCodeCriteria.maxSingleLineComments,
    maxMultiLineComments: stats.numberOfMultiLineComments <= cleanCodeCriteria.maxMultiLineComments
  };
};
...
|-----------------------------------|----------------|
| Max Function Length (lines <= 30) | ${criteria.maxFunctionLength ? 'Yes' : 'No'} |
| Max Classes per File (<= 5)       | ${criteria.maxClassesPerFile ? 'Yes' : 'No'} |
| Max Imports per File (<= 10)      | ${criteria.maxImportsPerFile ? 'Yes' : 'No'} |
| Max Single Line Comments (<= 20)  | ${criteria.maxSingleLineComments ? 'Yes' : 'No'} |
| Max Multi Line Comments (<= 10)   | ${criteria.maxMultiLineComments ? 'Yes' : 'No'} |
...
const main = async () => {
  try {
    ...
    for (const [category, reportContent] of Object.entries(categorizedReports)) {
      if (category !== 'server-files' && category !== 'client-files') { // Exclude specified categories
        const outputFilePath = path.join(outputDir, `${category}Eval.md`);
